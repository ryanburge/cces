...
male <-  cces16 %>% filter(race==1 & evangelical ==1 & complete.cases(vote16) & gender ==1 & complete.cases(pew_churatd)) %>%
  group_by(pew_churatd) %>% count(vote16, wt = commonweight) %>% 
  mutate(weight = prop.table(n)) %>% filter(vote16 ==1) %>% 
  mutate(label = c("Male"))

female <-  cces16 %>% filter(race==1 & evangelical ==1 & complete.cases(vote16) & gender ==2 & complete.cases(pew_churatd)) %>%
  group_by(pew_churatd) %>% count(vote16, wt = commonweight) %>% 
  mutate(weight = prop.table(n)) %>% filter(vote16 ==1) %>% 
  mutate(label = c("Female"))
...
male <-  cces16 %>% filter(race==1 & evangelical ==1 & complete.cases(vote16) & gender ==1 & complete.cases(agecut)) %>%
  group_by(agecut) %>% count(vote16, wt = commonweight) %>% 
  mutate(weight = prop.table(n)) %>% filter(vote16 ==1) %>% 
  mutate(label = c("Male"))


female <-  cces16 %>% filter(race==1 & evangelical ==1 & complete.cases(vote16) & gender ==2 & complete.cases(agecut)) %>%
  group_by(agecut) %>% count(vote16, wt = commonweight) %>% 
  mutate(weight = prop.table(n)) %>% filter(vote16 ==1) %>% 
  mutate(label = c("Female"))
...
mvote <-  cces16 %>% filter(race!=1 & evangelical ==1 & complete.cases(vote16) & gender ==1) %>%
  count(vote16, wt = commonweight) %>% 
  mutate(weight = prop.table(n)) %>% 
  mutate(label = c("Male"))

fvote <-  cces16 %>% filter(race!=1 & evangelical ==1 & complete.cases(vote16) & gender ==2) %>%
  count(vote16, wt = commonweight) %>% 
  mutate(weight = prop.table(n)) %>%  
  mutate(label = c("Female"))
...
vote %>% filter(vote16 == "Donald Trump" | vote16 == "Hillary Clinton" | vote16 == "Gary Johnson" | vote16 == "Jill Stein") %>% ggplot(., aes(1, weight)) + geom_col(aes(fill=fct_rev(vote16)), colour = "black") + coord_flip() + facet_grid(label ~.)  +
  ...
  ylab("Percent of Sample") + theme(legend.title=element_blank())  +
  theme(plot.title = element_text(hjust = 0.5)) + 
  theme(text=element_text(size=28, family="KerkisSans")) + ggtitle("Gender Differences in Non-White Evangelical Protestants") +
  labs(caption = "Data from CCES 2016") + 
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("chartreuse1", "gold1", "dodgerblue3", "firebrick1"))
...
total %>%  
  ...
  scale_x_discrete(labels = c("18-35", "36-51", "52-67", "68-83", "84-100")) + 
  scale_y_continuous(labels = scales::percent) + 
  labs(title = "Gender Differences Among Non White Evangelicals", x= "Age", y= "% of Two Party Vote for Trump", caption = "Date Source: CCES 2016") + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  theme(legend.position="bottom") + labs(fill="") + 
  theme(text=element_text(size=24, family="KerkisSans"))
...
male <-  cces16 %>% filter(race!=1 & evangelical ==1 & complete.cases(vote16) & gender ==1 & complete.cases(educ)) %>%
  group_by(educ) %>% count(vote16, wt = commonweight) %>% 
  mutate(weight = prop.table(n)) %>% filter(vote16 ==1) %>% 
  mutate(label = c("Male"))

female <-  cces16 %>% filter(race!=1 & evangelical ==1 & complete.cases(vote16) & gender ==2 & complete.cases(educ)) %>%
  group_by(educ) %>% count(vote16, wt = commonweight) %>% 
  mutate(weight = prop.table(n)) %>% filter(vote16 ==1) %>% 
  mutate(label = c("Female"))
...
total %>%  
  ...
  scale_x_discrete(labels = c("Less than HS", "HS Graduate", "Some College", "2 Yr Degree", "4 Yr Degree", "Postgraduate")) + 
  scale_y_continuous(labels = scales::percent) + 
  labs(title = "Gender Differences Among Non-White Evangelicals", x= "Education", y= "% of Two Party Vote for Trump", caption = "Date Source: CCES 2016") + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  theme(legend.position="bottom") + labs(fill="") + 
  theme(text=element_text(size=24, family="KerkisSans"))
