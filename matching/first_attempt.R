...
est <- att(mat, trump ~ treated + age + educ + income + attend + male + imp + pray + pid, model = "logit", data = match)

reg1 <- lm(trump ~ treated + age + educ + income + attend + male + imp + pray + pid, data = match)
...
dwplot(reg2, dodge_size = .05, show_intercept = FALSE) +
  theme_bw() +
