...
cces16 <- cces16 %>% 
  ...
  mutate(age2 = recode(age, "65:100 = 1; else = 99"))
...
all <- bind_df("aaa") %>% mutate(type = "Evangelical")

try1 <- all %>% 
  arrange(-mean) %>% 
  mutate(rank = seq(1:15))

all2 <- bind_df("aaa") %>% mutate(type = "Not Evangelicals")
...
gg2 <- newggslopegraph(rank, type, rank, issue, Title =  "Issue Importance Among White Respondents 65 and Older", SubTitle = "1 = Most Important, 15 = Least Important", Caption = "Data: CCES 2016", TitleTextSize = 44, SubTitleTextSize = 34, XTextSize = 34, YTextSize = 10, DataTextSize = 12, CaptionTextSize = 24)

ggsave("images/newslope2.png", typ = "cairo-png", gg2)

cces16 %>% 
  filter(age2 ==1 & evangelical !=1 & race ==1) %>% 
