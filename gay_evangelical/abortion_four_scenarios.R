...
ab_graph <- bind_rows(a1, a3, a4, a5) %>% 
  mutate(group = recode(new, "'evangelical' = 'Evangelical'; 'evanlgb' = 'Evangelical + LGB'; 'lgb' = 'LGB'")) %>% 
  select(-new) %>% 
...
cces16 <- cces16 %>% 
  mutate(imp_ab = recode(CC16_301b, "1=5; 2=4; 3=3; 4=2; 5=1; else=99")) %>% 
  mutate(imp_rel = recode(pew_religimp, "1=4; 2=3; 3=2; 4=1; else=99")) %>% 
  mutate(imp_ab = as.numeric(imp_ab), imp_rel = as.numeric(imp_rel))
...
cces16 %>% 
  ...
  filter(imp_rel < 99) %>% 
  filter(new != "NA") %>% 
  ggplot(., aes(x=imp_ab, y=imp_rel, fill = as.factor(new))) + 
  # geom_point(inherit.aes = TRUE) + 
  # geom_jitter(height = .25) + 
  geom_smooth(method = lm, inherit.aes = TRUE) +
  scale_fill_manual(values = Palette) + 
  theme_rb() +
  annotate("text", label = "Evangelical", x = 3.4, y = 3.55, color = "black", size =8, family = "IBM Plex Serif") +
  annotate("text", label = "LGB Evangelical", x = 3.3, y = 3.3, color = "black", size =8, family = "IBM Plex Serif") +
  annotate("text", label = "LGB", x = 3.5, y = 2.28, color = "black", size =8, family = "IBM Plex Serif") +
  labs(x= "Importance of Abortion", y = "Importance of Religion", title = "Relationship between Abortion and Religion Importance", caption = "Data: CCES 2016")  + 
...
ggsave(file="D://cces/gay_evangelical/scatter_ab_.png", type = "cairo-png", width = 20, height =12)
