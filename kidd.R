...
a2<- cces16 %>% 
  filter(pew_bornagain ==1 & religpew ==1 & race ==1) %>% 
...
a2 %>% 
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(x= "Self Described Party Identity", y= "Percent of Sample", title = "Partisan Breakdown of White Born Again Protestants", caption = "Data: CCES 2016" ) +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text=element_text(size=40, family="KerkisSans"))   +
  theme(legend.position = "none") + theme(legend.title=element_blank()) + 
  scale_fill_manual(values=c("dodgerblue3", "darkgrey", "firebrick1")) +
  theme(plot.title = element_text(face="bold")) + theme_rb()

ggsave(file="baprot_pid3_white.png", type = "cairo-png", width = 20, height =12)
