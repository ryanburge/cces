...
cces16 <- cces16 %>% 
  mutate(ortho = recode(religpew, "4=1; else =0"))

## Jehovah's Witness

cces16 <- cces16 %>% 
  mutate(witness = recode(religpew_protestant, "13=1; else =0"))

## Mainline Protestants

cces16 <- cces16 %>% 
  mutate(protestant = recode(religpew, "1=1; else=0"), notba = recode(pew_bornagain, "2=1; else=0")) %>% 
  mutate(mline = protestant + notba) %>% 
  mutate(mline = recode(mline, "2=1; else=0"))

cces16 <- cces16 %>% 
  mutate(catholic = recode(religpew, "2=1; else=0"))


g1 <- cces16 %>% select(V101, evangelical, mormon, ortho, witness, mline, catholic) %>% 
  gather(new, x1, evangelical:catholic) %>% 
